...
float SphereTrace(float t){
	t = abs(t - 10.);
    t = 0.01*t;
    return clamp(t*t, 50./iResolution.y, 80.);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float x = 10. + 10.*cos(0.002*iGlobalTime);
    float y = 2.+0.25*sin(5.*iGlobalTime);
    float z = 8. + 400.*sin(0.002*iGlobalTime) + 6.*mouse.x;
    ...
    vec3 lPos = normalize(vec3(0.2, .2, 0.2));//*mat3(cos(0.05*iGlobalTime), sin(0.05*iGlobalTime), 0.,
                                                 // -sin(0.05*iGlobalTime), cos(0.05*iGlobalTime), 0.,
                                                  //0., 0., 0.));
    if( !Scene(ro, rd, distance, fragCoord )|| distance > 400.){
        ...
        //else
        {
            ...
            //col*=0.7;
            //col += pow(0.1*Noise((ro+rd*t).xz), 1000.); stars try
        ...
        col = Render(ro, rd, distance, normal);//TerrainColour(ro, pos, normal, distance, fragCoord);
        float effect = 0.;//= 0.3;
        ...
        vec3 dir = normalize(lPos - (ro+rd*t));
       	float h = 0.;
        if(Scene2((ro+rd*t), dir, t, fragCoord, h)){
        	col *= 0.8;
