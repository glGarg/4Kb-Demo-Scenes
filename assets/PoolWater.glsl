float WaveAmplitude() {
	return .04*exp(-.02*mod(iGlobalTime, 30.));
}

float WaterWaves(vec3 a) {
	return WaveAmplitude() * sin((2. * a.x * a.x + 2. * a.z * a.z) - 10.*mod(iGlobalTime, 30.));
}
...
float Ball(vec3 pos){
	return length(pos - sin(3.*iGlobalTime)*vec3(0., .9, 0.)) - .75;
}
