...
vec3 Trace(vec3 pos, vec3 ro, vec3 rd, float t, float closest)
{
    ...
    col += spec + max(sin(9.*iGlobalTime)*spec, 0.);
    float atten =0.1;
...
{
    //mat3 m2 = Quaternion(normalize(vec3(0.1, 1.0, 0.01)), sin(.9*iGlobalTime));
    m = Quaternion(normalize((vec3(0.1*sin(2.*iGlobalTime), 1.5*cos(0.1*iGlobalTime), 1.7*tan(0.1*iGlobalTime)))),.02*iMouse.y);
	vec2 p = 2.*fragCoord.xy / iResolution.xy - 1.;
	p.x *= iResolution.x/iResolution.y;
    
    float x =1.*(mod(abs(iMouse.x*0.003+.05*iGlobalTime), 10.));//-1.*(sin(iGlobalTime));
    float y = -1.;
    float z = 1.2*sqrt(100.-pow(x, 2.));//-1.*(cos(iGlobalTime));
    vec3 ro = vec3(x, y, z);
    vec3 lookAt = vec3(0.);
