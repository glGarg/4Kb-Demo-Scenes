...
vec3 TerrainCol(vec3 ro, vec3 pos, vec3 normal, float dis)
{
    ...
    float fog = exp(-.001 * dis*dis * sin(iGlobalTime));
...
vec4 Map(vec3 p)
{
	...
	f = texture2D(iChannel0, .08*p.xz, 4.*(sin(f.a)*.5+.5));
...
{
    vec4 col = vec4(texture2D(iChannel0, -.3*p.xz+.5).xyz, 1.);
    //col *= 1.5*frequency;
    vec3 normal = GetNormal(p);
    vec4 f;
    g /= iResolution.xy * vec2(12.,1.2);
	g.y += sin((g.x -= (f.a = .3) * .015) * 46.5 + f.a) * .12;
	f = texture2D(iChannel0, .05*p.xz, 4.*(sin(f.a)*.5+.5));
	f = smoothstep(f+.5, f, f/f*.71);
    //shine += .01;
...
vec3 Scene(vec3 ro, vec3 rd)
{
    ...
    for (int i = 0; i < 900; i++) {
    	if (t > 10. || d < 0.) {
            return .2+100.*vec3(pow(texture2D(iChannel2, a.xz).xyz, vec3(100.)));
        }
        vec4 a = Map(ro + rd*t);
        d = a.x;
        if (d < 0.001) {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    uv.y -= 1.*iMouse.y/iResolution.y-.5;
    if (abs(uv.x) > .8) {
    	fragColor = vec4(0., 0., 0., 1.);
        return;
    }
    g = fragCoord;
    mat3 m = Quaternion(normalize(vec3(1., 3.5, 0.)), iGlobalTime*.1);
    float x = 0.2;
    float y = 5.5;
    float z = .3;
    vec3 ro = vec3(x, y, z);
    ...
    a = mat3(normalize(cross(normalize(vec3(-.2, -5.5, -.3)), vec3(0., 1., 0.)))/.58,
             ...
             normalize(vec3(-.2, -5.5, -.3)))*m*normalize(vec3(uv/2., 0.3));
