...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float x = 10. + 3.*iGlobalTime;
    float y = 5.;//+iMouse.y;
    float z = 8. + 3.*iGlobalTime + iMouse.x;
    
    vec3 ro = vec3(x, y, z);
    ro.y += Terrain(ro.xz, 5);
    
    vec3 lookAt = vec3(ro.x, ro.y*0.5, 0.);
    ...
    if( !Scene(ro, rd, distance, fragCoord )|| distance > 500.){
    	fragColor = vec4(0.7, 0.7, 0.72, 0.);
        float fog = exp(-.0005 * distance*distance);
		fragColor = mix(vec4(0.9, 0.9, 0.9, 1.), fragColor, fog);
        return;
    }
    else{
        ...
        col = TerrainColour(ro, pos, normal, distance, fragCoord);
